...
public class CheckoutSteps extends StepUtils {
    ...
    public void I_checkout_until_I_reach_the_page_as_a_user(String pageName, String userType, String country, String address) throws Throwable {
        ...
        if(address != null){
            if(address.equals("paypal_address"))
    ...
    public void I_fill_in_payment_information_on_guest_payment_page() throws Throwable {
        HashMap<String, String> opts = new HashMap<>();
        opts.put("checkout_eligible", "true");
...
//        }
    ...
    public void I_select_pick_up_option_for_bops_item() throws Throwable {
        ...
        if (!MEW())
            Assert.assertTrue(Elements.elementPresent("shopping_bag.pick_it_up_button"), "ERROR-DATA: Selected Product is not a BOPS Eligible one");
    ...
    public void I_select_sdd_shipping_in_shipping_methods() throws Throwable {
        if (signedIn()) {
            ...
            if(onPage("responsive_checkout_signed_in")){
                ...
                Assert.assertTrue(Wait.untilElementPresent("responsive_checkout_signed_in.sdd_shipping"), "ERROR-DATA: Added Product or Address is not a sdd eligible one");
                ...
                Assert.assertTrue(Elements.elementPresent("shipping_payment_signed_in.sdd_shipping"), "ERROR-DATA: Added Product or Address is not a sdd eligible one");
            ...
            Assert.assertTrue(Elements.elementPresent(page + ".sdd_shipping"), "ERROR-DATA: Added Product or Address is not a sdd eligible one");
    ...
    public void I_apply_promo_code_using_website(String validity, String promo_code) throws Throwable {
        try {
            Assert.assertTrue(Wait.untilElementPresent("shopping_bag.promo_code"), "ERROR: promo code field is not present");
        ...
        if (validity.equals("valid"))
            Assert.assertTrue(Wait.untilElementPresent("shopping_bag.promo_code_remove"), "ERROR-DATA: Not a valid promo code");
        else
            Assert.assertTrue(Wait.untilElementPresent("shopping_bag.error_message"), "ERROR-DATA: Not an invalid promo code");
    }
    ...
    public void I_should_see_following_delivery_ship_note_for_same_day_delivery_shipping_method_on_page(String display, String pageName, List<String> message) throws Throwable {
        ...
        if(!page.equals("responsive_order_review_section"))
            Assert.assertTrue((display.contains("normal") ? Elements.getText(page+".normal_shipping_note") : Elements.getText(page+".sdd_shipping_note")).contains(message.get(message.size()-1)), "Expected delivery ship note not displayed!!");
    ...
    public void I_should_see_following_estimated_delivery_message_on_page(String pageName, List<Map<String, String>> message) throws Throwable {
        ...
        Assert.assertTrue((actualShippingText.contains(expectedShippingText)), "Expected delivery message is not displayed!!");
    }
    ...
    public void I_select_method_in_shipping_methods(String shippingMethod) throws Throwable {
        ...
        Wait.forPageReady();
        if(signedIn())
            Wait.secondsUntilElementPresentAndClick(page + ".change_shipping_method", 5);
        Assert.assertTrue(Wait.secondsUntilElementPresent(page + "." + type + shippingMethod + "_shipping", 5), "ERROR-DATA: Shipping method is not displayed as product is not eligible for that method");
        Clicks.javascriptClick(page + "." + type + shippingMethod + "_shipping");
        pausePageHangWatchDog();
        shippingDetails = ((HashMap<String, HashMap>) (new Checkout().getShippingMethods())).get(shippingMethod);
        resumePageHangWatchDog();
        if (signedIn())
            Clicks.click("responsive_checkout_signed_in.save_shipping_method");
    }
