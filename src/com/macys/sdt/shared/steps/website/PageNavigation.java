...
public class PageNavigation extends StepUtils {
    ...
    @Given("^I visit the web site as a (guest|registered) user in (mMoney|bMoney) (earn|burn) period$")
    public void I_visit_the_web_site_as_a_guest_user_in_mMoney_earn_period(String userType, String moneyType, String campaignType) throws Throwable {
        if (campaignType.equals("earn") && CampaignService.getAllMbmoneyCampaignPeriods().get("earn").equals(false))
            CampaignService.updateMbmoneyCampaignDetailsInDatabase(macys() ? "MEarn" : "BEarn");
        else if (campaignType.equals("burn") && CampaignService.getAllMbmoneyCampaignPeriods().get("redeem").equals(false))
            CampaignService.updateMbmoneyCampaignDetailsInDatabase(macys() ? "MRedeem" : "BRedeem");
        else if (CampaignService.getAllMbmoneyCampaignPeriods().get(campaignType).equals(false))
            CampaignService.updateMbmoneyCampaignDetailsInDatabase(campaignType);
